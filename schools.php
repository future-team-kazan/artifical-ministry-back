<?php
// В ЭТОМ МОДУЛЕ МЫ ВЫГРУЖАЕМ ИНФОРМАЦИЮ О ШКОЛАХ, КОТОРАЯ В FRONT-END БУДЕТ
// ПРЕДСТАВЛЕНА В ВИДЕ ТАБЛИЦЫ ПО ВСЕМ УЧРЕЖДЕНИЯМ С КОНТРОЛИРУЕМЫМ ПОКАЗАТЕЛЕМ
// - СРЕДНЕЙ ЗАРПЛАТОЙ ПО ВСЕМ СОТРУДНИКАМ
...
for ($i = 0; $i < $rows_schools; $i++) 
{
		...
		//SELECT vr.* FROM iministry.values_reports vr LEFT JOIN iministry.values_reports vrj ON vr.Period = vrj.Period AND vr.Period < vrj.Period WHERE vr.Organization = 1 ORDER BY Period DESC LIMIT 1
...
//echo $value;
//echo $school_id;
$school_data_temp1 = array(
"name" => $school_name,
"value" => $value
);


$schools_data_temp[$i] = array(
"id" => $school_id,
"school_data" => $school_data_temp1
);
